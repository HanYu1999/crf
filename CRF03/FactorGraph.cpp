...
{
	for (int i = 0; i < 2; i++) // one factor node can only have 2 neighbor
	{
		...
		else
		{
			for (int y = 0; y < num_label; y++)
			{
				...
				for (int y1 = 0; y1 < num_label; y1++)
					s += func->GetValueByAttr(neighbor[i], neighbor[1-i], y, y1) * belief[1 - i][y1];
				msg[y] = s;
			}
			NormalizeMessage();
		}
...
void FactorNode::MaxSumPropagation(double* diff_max, bool labeled_given)
{
	for (int i = 0; i < 2; i++)    
	{
		...
		else
		{
			for (int y = 0; y < num_label; y++)
			{
				double s = func->GetValueByAttr(neighbor[i], neighbor[1-i], y, 0) * belief[1 - i][0];
				double tmp;
				for (int y1 = 0; y1 < num_label; y1 ++)
				{
					tmp = func->GetValueByAttr(neighbor[i], neighbor[1-i], y, y1) * belief[1 - i][y1];
					if (tmp > s) s = tmp;
				}
				msg[y] = s;
			}
