...
void CRFModel::GenFeature() {
    ...
    for (int y1 = 0; y1 < num_label; y1++)
        for (int y2 = y1; y2 < num_label; y2++) {
            // TODO: 1, change the length of lambda
//            edge_feature_offset.insert(make_pair(y1 * num_label + y2, offset));
//            offset++;
            for(int x1 = 0; x1 < num_attrib_type; x1 ++){
                // TODO change here
                for(int x2 = 0; x2 < num_attrib_type; x2++){
                    edge_feature_offset.insert(make_pair((y1 * num_label + y2) * num_attrib_type * num_attrib_type + x1 * num_attrib_type + x2, offset));
                    offset ++;
                }
            }
        }
...
void CRFModel::SetupFactorGraphs() {
    ...
    func_list = new EdgeFactorFunction *[num_edge_type]; // yh: the transfer function
    for (int i = 0; i < num_edge_type; i++) {
        // TODO 5, add a param
        // func_list[i] = new EdgeFactorFunction(num_label, p_lambda, &edge_feature_offset);
        func_list[i] = new EdgeFactorFunction(num_label, num_attrib_type, p_lambda, &edge_feature_offset);
    ...
    for (int s = 0; s < num_sample; s++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // TODO I am trying to correct the bp
            sample_factor_graph[s].var_node[i].data = sample->node[i];
        }

        // Add edge info
        for (int i = 0; i < m; i++) {
            sample_factor_graph[s].AddEdge(sample->edge[i]->a, sample->edge[i]->b,
                                           func_list[sample->edge[i]->edge_type]);
        }
...
//    double *tmp_store = new double[num_feature + 1];
    ...
    // TODO 2, l1
    this->alpha_node = conf->alpha_node;
    this->alpha_edge = conf->alpha_edge;
...
double CRFModel::CalcPartialLabeledGradientForSample(DataSample *sample, FactorGraph *factor_graph, double *gradient) {
    ...
    // TODO 5
    for (int i = 0; i < num_edge_type; i++){
        func_list[i]->recompute_combined();
    }
...
//                    std::cout<<p_lambda[sample->node[i]->attrib[t]]<<std::endl;
//                    std::cout<<sample->node[i]->value[t]<<std::endl;
//                    std::cout<<p_lambda[sample->node[i]->attrib[t]] * sample->node[i]->value[t]<<std::endl;
//                    std::cout<<exp(p_lambda[sample->node[i]->attrib[t]] * sample->node[i]->value[t])<<std::endl;
    ...
    */
    ...
    for (int i = 0; i < n; i++) {
        for (int y = 0; y < num_label; y++) {
            for (int t = 0; t < sample->node[i]->num_attrib; t++) {
                // TODO 2, l1, lambda
                if (lambda[GetAttribParameterId(y, sample->node[i]->attrib[t])] < 0)
                    gradient[GetAttribParameterId(y, sample->node[i]->attrib[t])] += alpha_node;
                else if (lambda[GetAttribParameterId(y, sample->node[i]->attrib[t])] > 0)
                    gradient[GetAttribParameterId(y, sample->node[i]->attrib[t])] -= alpha_node;

                gradient[GetAttribParameterId(y, sample->node[i]->attrib[t])] +=
                        sample->node[i]->value[t] * factor_graph->var_node[i].marginal[y];
            }
        }
    }
    ...
    for (int i = 0; i < m; i++) {
        for (int a = 0; a < num_label; a++)
            for (int b = 0; b < num_label; b++) {
                // TODO: 1, change because of the change of length
//                gradient[GetEdgeParameterId(sample->edge[i]->edge_type, a,
//                                            b)] += factor_graph->factor_node[i].marginal[a][b];
                DataEdge* e = sample->edge[i];
                DataNode* node1 = sample->node[e->a];
                DataNode* node2 = sample->node[e->b];
                for(int x1 = 0; x1 < node1->num_attrib; x1 ++){
                    for(int x2 = 0; x2 < node2->num_attrib; x2++){
//                        std::cout<<GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])<<std::endl;
//                        std::cout<<"x1:"<<node1->attrib[x1]<<std::endl;
//                        std::cout<<"x2:"<<node2->attrib[x2]<<std::endl;
                        // TODO 2, l1
                        if (lambda[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])] < 0)
                            gradient[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])] += alpha_edge;
                        else if (lambda[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])] > 0)
                            gradient[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])] -= alpha_edge;

                        gradient[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])]
                                += factor_graph->factor_node[i].marginal[a][b] * node1->value[x1] * node2->value[x2];
                    }
                }
    ...
    for (int i = 0; i < m; i++) {
        for (int a = 0; a < num_label; a++)
            for (int b = 0; b < num_label; b++) {
                // TODO:1, change because of the length
//                gradient[GetEdgeParameterId(sample->edge[i]->edge_type, a,
//                                            b)] -= factor_graph->factor_node[i].marginal[a][b];
                DataEdge* e = sample->edge[i];
                DataNode* node1 = sample->node[e->a];
    ...
    for (int i = 0; i < m; i++) {
        ...
        // TODO: 1, change the way of likelihood
//        f += lambda[this->GetEdgeParameterId(sample->edge[i]->edge_type, a, b)];
        ...
        for(int x1 = 0; x1 < node1->num_attrib; x1 ++){
            for(int x2 = 0; x2 < node2->num_attrib; x2++){
                f += lambda[this->GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])]
                        * node1->value[x1] * node2->value[x2];
            }
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        for (int y = 0; y < num_label; y++) {
            for (int t = 0; t < sample->node[i]->num_attrib; t++)
                Z += lambda[this->GetAttribParameterId(y, sample->node[i]->attrib[t])] * sample->node[i]->value[t] *
                     factor_graph->var_node[i].marginal[y];
        }
    }
    for (int i = 0; i < m; i++) {
        for (int a = 0; a < num_label; a++)
            for (int b = 0; b < num_label; b++) {
                // TODO: 1, change because of length
//                Z += lambda[this->GetEdgeParameterId(sample->edge[i]->edge_type, a, b)] *
//                     factor_graph->factor_node[i].marginal[a][b];
                ...
                for(int x1 = 0; x1 < node1->num_attrib; x1 ++){
                    for(int x2 = 0; x2 < node2->num_attrib; x2++){
                         Z += lambda[GetEdgeParameterId(sample->edge[i]->edge_type, a, b, node1->attrib[x1], node2->attrib[x2])]
                                * factor_graph->factor_node[i].marginal[a][b] * node1->value[x1] * node2->value[x2];
                    }
                }
            }
        ...
        for (int a = 0; a < num_label; a++)
            for (int b = 0; b < num_label; b++) {
                if (factor_graph->factor_node[i].marginal[a][b] > 1e-10)
                    h_e += -factor_graph->factor_node[i].marginal[a][b] *
                           log(factor_graph->factor_node[i].marginal[a][b]);
            }
        Z += h_e;
    }
    // Node entroy
    ...
    // TODO 2, l1
    double alpha = alpha_node;
    for(int i = 0; i < num_feature; i++){
        if (i >= num_attrib_parameter)
            alpha = alpha_edge;
        if(lambda[i] < 0)
            f += alpha * lambda[i];
        else
    ...
    // TODO 3 management
    FILE *pred_out = fopen((conf->out_dir + "/pred.txt").c_str(), "a");
    fflush(pred_out);

    for (int i = 0; i < n; i++) {
        fprintf(pred_out, "%s\n", train_data->label_dict.GetKeyWithId(inf_label[i]).c_str());
//        // TODO not finish here
//        if (sample->node[i]->label_type==Enum::UNKNOWN_LABEL && sample->node[i]->type1==Enum1::NOT)
//            continue;
    ...
    // TODO : the evaluation below just work for binary case
    ...
    // TODO: add these code below to store the parameter
    // FILE *fparam = fopen("parameter.txt", "a");
    // TODO 3, management
    FILE *fparam = fopen((conf->out_dir + "/parameter.txt").c_str(), "a");
    //double * cur_lambda = lambda;
    fprintf(fparam, "*********************************\n");
    fprintf(fparam, "          STATE PARAM\n");
    fprintf(fparam, "*********************************\n");
    for (int i = 0; i < num_label; i++){
        fprintf(fparam, "label %s::\n", this->train_data->label_dict.GetKeyWithId(i).c_str());
        for (int j = 0; j < num_attrib_type; j++){
            if (lambda[GetAttribParameterId(i, j)] == 0) continue;
            fprintf(fparam, "%s: %.10f ", this->train_data->attrib_dict.GetKeyWithId(j).c_str(), lambda[GetAttribParameterId(i, j)]);
        }
        fprintf(fparam, "\n");
    }
    fprintf(fparam, "*********************************\n");
    fprintf(fparam, "        TRANSFER PARAM\n");
    fprintf(fparam, "*********************************\n");
    for (int i = 0; i < num_edge_type; i++){
        fprintf(fparam, "####edge_type: %s::\n", this->train_data->edge_type_dict.GetKeyWithId(i).c_str());
        for (int label1 = 0; label1 < num_label; label1++){
            for (int label2 = label1; label2 < num_label; label2++){
                fprintf(fparam, "label %s -- label %s:\n", this->train_data->label_dict.GetKeyWithId(label1).c_str(), this->train_data->label_dict.GetKeyWithId(label2).c_str());
                for(int x1 = 0; x1 < num_attrib_type; x1 ++){
                    for(int x2 = 0; x2 < num_attrib_type; x2 ++){
                        double value = lambda[GetEdgeParameterId(i, label1, label2, x1, x2)];
                        if (value == 0) continue;
                        fprintf(fparam, "%s - %s: %.10f  ", this->train_data->attrib_dict.GetKeyWithId(x1).c_str(), this->train_data->attrib_dict.GetKeyWithId(x2).c_str(), value);
                    }
                }
                fprintf(fparam, "\n");
            }
...
void CRFModel::LoadModel(const char *filename) {
    ...
    for (;;) {
        ...
        if (tokens[0] == "#edge") {
            ...
            // TODO: 1, just for now
            int i = GetEdgeParameterId(type_id, id1, id2, 0, 0);
            lambda[i] = value;
        }
    }
    fclose(fin);
    ...
    for (int T = 0; T < num_edge_type; T++) {
        ...
        for (int i = 0; i < num_label; i++) {
            ...
            for (int j = i; j < num_label; j++) {
                ...
                // TODO, 1, just for now
		// TODO: change
                int pid = GetEdgeParameterId(T, i, j, 0, 0);
