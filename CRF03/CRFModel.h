...
//         TODO 5
    ...
    virtual double GetValueByAttr(Node* n1, Node* n2, int y1, int y2){
        DataNode* n_min, * n_max;
        int y_min, y_max;
        double res = 0.0;
        if (y1 < y2){
            y_min = y1; y_max = y2;
            n_min = ((VariableNode*)n1)->data; n_max = ((VariableNode*)n2)->data;
        }else{
            y_min = y2; y_max = y1;
            n_min = ((VariableNode*)n2)->data; n_max = ((VariableNode*)n1)->data;
        }
        for(int a1 = 0; a1 < n_min->num_attrib; a1 ++){
            for(int a2 = 0; a2 < n_max->num_attrib; a2 ++){
                res += lambda[(*feature_offset)[(y_min * num_label + y_max) * num_attr * num_attr + n_min->attrib[a1] * num_attr + n_max->attrib[a2]]];
            }
        }
        return exp(res/n_min->num_attrib/n_max->num_attrib);
    }
