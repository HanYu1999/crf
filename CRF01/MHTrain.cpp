		...
		for (int batch_iter = 0; batch_iter < iters; batch_iter++)
		{
			...
			for (it = change.begin(); it != change.end(); it++)
			{
				...
				for (int j = 0; j < U->neighbor.size(); j++)
				{
					...
					int v = V->id;
					// TODO, 1, just for now
					int pid = GetEdgeParameterId(sample->edge[w]->edge_type, _state[u], _state[v], 0, 0);
					likeli += lambda[pid];
	...
	for (int iter = 0; iter < iters; iter++)
	{
		...
		for (it = change.begin(); it != change.end(); it++)
		{
			...
			for (int j = 0; j < U->neighbor.size(); j++)
			{
				...
				int v = V->id;
				// TODO, 1, just for now
				int pid = GetEdgeParameterId(sample->edge[w]->edge_type, state[u], state[v], 0, 0);
				likeli += lambda[pid];
			}
        }
	...
	for (int i = 0; i < unlabeled.size(); i++)
	{
		...
		for (int k = 0; k < num_label; k++)
		{
			...
			for (int j = 0; j < U->neighbor.size(); j++)
			{
				...
				int v = V->id;
				// TODO, 1, just for now
				int pid = GetEdgeParameterId(sample->edge[w]->edge_type, k, best_state[v], 0, 0);
				p[k] += lambda[pid];
			}
		}
		int ynew = 0;
