...
//            offset++;
            for(int x1 = 0; x1 < num_attrib_type; x1 ++){
                for(int x2 = x1; x2 < num_attrib_type; x2++){
...
//    FILE *pred_out = fopen("pred.txt", "w");
    FILE * pred_out = fopen("/Users/athena/CLionProjects/ForCRF/CRF/OpenCRF/OpenCRF/pred.txt", "a");
    fflush(pred_out);
    ...
    FILE *fparam = fopen("/Users/athena/CLionProjects/ForCRF/CRF/OpenCRF/OpenCRF/parameter.txt", "a");
    double * cur_lambda = lambda;
    fprintf(fparam, "*********************************\n");
    fprintf(fparam, "          STATE PARAM\n");
    fprintf(fparam, "*********************************\n");
    for (int i = 0; i < num_label; i++){
        fprintf(fparam, "label %d::\n", i);
        for (int j = 0; j < num_attrib_type; j++){
            if (cur_lambda[j] == 0) continue;
            fprintf(fparam, "att%d: %f ", j,cur_lambda[j]);
        }
        fprintf(fparam, "\n");
        cur_lambda = cur_lambda + num_attrib_type;
    }
    fprintf(fparam, "*********************************\n");
    fprintf(fparam, "        TRANSFER PARAM\n");
    fprintf(fparam, "*********************************\n");
    for (int i = 0; i < num_edge_type; i++){
        fprintf(fparam, "edge_type: %d::\n", i);
        for (int label1 = 0; label1 < num_edge_type; label1++){
            for (int label2 = label1; label2 < num_edge_type; label2++){
                double value = cur_lambda[edge_feature_offset[label1 * num_edge_type + label2]];
                fprintf(fparam, "label %d -- label %d: %f   ", label1, label2, value);
            }
            fprintf(fparam, "\n");
        }
        fprintf(fparam, "\n");
        cur_lambda = cur_lambda + num_edge_feature_each_type;
    }
    fflush(fparam);
