...
// TODO: not suitable for gcn dataset
...
//            curt_edge->b = atoi(tokens[2].c_str()) - 1;
        ...
        } else //node
        {
            ...
            else if (label_type == '&') {  // TODO add here
                curt_node->label_type = Enum::UNKNOWN_LABEL;
                curt_node->type1 = Enum1::NOT;
            }
            else {
...
void DataSet::LoadDataWithDict(const char *data_file, Config *conf, const MappingDict &ref_label_dict,
                               const MappingDict &ref_attrib_dict, const MappingDict &ref_edge_type_dict) {
    char buf[MAX_BUF_SIZE];
    DataSample *curt_sample = new DataSample();
    char *eof;
