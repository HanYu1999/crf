...
/*********************************************************************
 IdKlDict
*********************************************************************/
void IdKlDict::sort_id_by_value(bool is_ascending){
	if(is_ascending)
		std::sort(nodes.begin(), nodes.end(), compare_ascend);
	else
		std::sort(nodes.begin(), nodes.end(), compare_descend);
//	for(int i = 0; i < node_id.size(); i++)
//		kl_value[i] = id_kl_map[node_id[i]];
}

bool IdKlDict::compare_ascend(NodeKL* a1, NodeKL* a2){
    return a1->kl_value < a2->kl_value;
}

bool IdKlDict::compare_descend(NodeKL* a1, NodeKL* a2){
    return a1->kl_value > a2->kl_value;
}